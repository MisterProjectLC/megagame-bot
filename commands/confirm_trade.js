...
module.exports = {
    ...
    execute: async (com_args, msg, send_message) => {
        await db.makeQuery(`SELECT * FROM trocas WHERE ofertado = (SELECT time_nome FROM jogadores WHERE jogador_id = $1) AND ofertante ILIKE $2`, [msg.author.id, com_args[0]]).then(async (response) => {
